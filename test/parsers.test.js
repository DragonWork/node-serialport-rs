...
const {DelimiterParser, ReadlineParser, ByteLengthParser, ReadyParser, SlipEncoder, SlipDecoder,
  CCTalkParser, PacketLengthParser, SpacePacketParser, RegexParser, InterByteTimeoutParser} = require('..');
...
test('delimiter spanning chunks preserves the incomplete tail', async () => {
  ...
  assert.deepEqual(output.map(chunk => chunk.toString()), ['ab', 'cd', 'e']);
});
...
test('byte-length handles chunk boundaries and trailing bytes', async () => {
  ...
  assert.deepEqual(output.map(chunk => [...chunk]), [[0, 1, 2], [3, 4, 5], [6]]);
});
...
test('delimiter framing is independent of chunk boundaries and overlapping prefixes', async () => {
  ...
  for (const includeDelimiter of [false, true]) {
    ...
    for (let at = input.indexOf(delimiter); at !== -1; at = input.indexOf(delimiter, offset)) {
      if (at > offset || includeDelimiter) expected.push(input.subarray(offset, at + (includeDelimiter ? delimiter.length : 0)));
...
test('SLIP retains the documented invalid-escape and trailing-frame behavior', async () => {
  assert.deepEqual(await parse(new SlipDecoder(), [Buffer.from([7, 0xdb]), Buffer.from([9, 0xc0, 10])]),
    [Buffer.from([7]), Buffer.from([9]), Buffer.from([10])]);
...
test('length-prefixed frames handle multiple multibyte delimiters and split length fields', async () => {
  const options = {delimiter: [0xabc123, 0x987654], delimiterBytes: 3, lengthOffset: 3, lengthBytes: 2, packetOverhead: 5, maxLen: 1024};
...
test('length-prefixed frames emit an oversized header early and recover', async () => {
  ...
  assert.deepEqual(await parse(new PacketLengthParser({maxLen: 4}), fragments(input, 1)),
    [Buffer.from([0xaa, 6]), Buffer.from([0xaa, 2, 3, 4]), Buffer.from([0xaa, 4, 5])]);
});
...
test('space packets expose the header and secondary fields without recursive parsing', async () => {
  ...
  const expected = {
    header: {versionNumber: 1, identification: {apid: 0x123, secondaryHeader: 1, type: 0},
      sequenceControl: {packetName: 7, sequenceFlags: 3}, dataLength: 5},
    data: 'CD', secondaryHeader: {timeCode: 'T', ancillaryData: 'AB'},
  };
  for (let size = 1; size <= frame.length; size++) {
    assert.deepEqual(await parse(new SpacePacketParser({timeCodeFieldLength: 1, ancillaryDataFieldLength: 2}), fragments(frame, size)), [expected, []]);
  }
  const parser = new SpacePacketParser();
  let count = 0;
  parser.on('data', packet => { if (!Array.isArray(packet)) count++; });
...
test('timeout parser emits bounded frames, flushes the tail, and has no timer after destroy', async () => {
  const output = await parse(new InterByteTimeoutParser({interval: 10, maxBufferSize: 3}), [Buffer.from('ab'), Buffer.from('cdefg')]);
  assert.deepEqual(output.map(buffer => buffer.toString()), ['abc', 'def', 'g']);
  ...
  assert.deepEqual(received.map(buffer => buffer.toString()), ['first']);
});
...
test('parser constructors reject invalid frame sizes instead of retaining bytes forever', () => {
  ...
  for (const delimiter of [undefined, '', Buffer.alloc(0)]) assert.throws(() => new DelimiterParser({delimiter}), TypeError);
