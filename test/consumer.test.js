...
for (const operation of ['read', 'write']) {
  test(
    `original serialport stream reports a Rust binding ${operation} disconnect`,
    {
...
test(
  'herdsman serial adapter transfers bytes and reports a disconnect',
  {
  ...
  async (t) => {
    ...
    assert(
      SerialPort.prototype instanceof implementation.SerialPortStream,
      'Consumer must resolve the selected stream',
    );
    const terminal = await pty(t);
