...
test('discovery returns the complete metadata shape without opening ports', async () => {
  ...
  for (const port of ports) {
    ...
    assert.deepEqual(Object.keys(port).sort(), ['locationId', 'manufacturer', 'path', 'pnpId', 'productId', 'serialNumber', 'vendorId']);
  }
});
...
test('a disconnected binding remains closable until its consumer acknowledges the failure', {timeout: 5000}, async t => {
  const terminal = await pty(t);
  const binding = await RustBinding.open({path: terminal.path, baudRate: 115200});
...
test('a native callback allocation failure closes once and releases the exclusive port lock', {timeout: 5000}, async t => {
  const terminal = await pty(t);
  ...
  } finally { Buffer.allocUnsafe = allocator; }
  t.after(() => port.destroy());
...
test('a detached read destination rejects instead of reporting bytes it did not receive', {timeout: 5000}, async t => {
  const terminal = await pty(t);
  const binding = await RustBinding.open({path: terminal.path, baudRate: 115200});
  t.after(() => binding.isOpen && binding.close());
...
test('shrinking a read destination is rejected before copying into it', {
  timeout: 5000, skip: typeof ArrayBuffer.prototype.resize !== 'function',
}, async t => {
  const terminal = await pty(t);
  const binding = await RustBinding.open({path: terminal.path, baudRate: 115200});
  t.after(() => binding.isOpen && binding.close());
