...
if (target && !/^[a-z0-9_]+(?:-[a-z0-9_]+)+$/.test(target))
  throw new Error('Pass a Rust target triple in CARGO_BUILD_TARGET');
withBuildLock(root, () => {
  ...
  const library =
    process.platform === 'win32'
      ? 'node_serialport_rs.dll'
      : process.platform === 'darwin'
        ? 'libnode_serialport_rs.dylib'
        : 'libnode_serialport_rs.so';
  ...
  try {
    ...
    const checked = spawnSync(
      process.execPath,
      ['-e', 'if (typeof require(process.argv[1]).NativePort !== "function") process.exit(1)', staged],
