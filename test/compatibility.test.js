...
for (const [name, SerialPortStream] of implementations) {
  function stream(t, overrides = {}) {
    ...
    const port = new SerialPortStream({path: 'test-device', baudRate: 115200, autoOpen: false,
      binding: {async open() { return binding; }}});
    port.on('error', () => {});
  ...
  test(`${name}: set supplies default flags without changing caller options`, async t => {
    const requests = [];
    const port = stream(t, {async set(options) { requests.push(options); }});
  ...
  for (const afterFinish of [false, true]) {
    test(`${name}: drain ${afterFinish ? 'after finish' : 'during end'} waits for output`, {timeout: 3000}, async t => {
      let releaseWrite;
      const writing = new Promise(resolve => { releaseWrite = resolve; });
      const order = [];
      const port = stream(t, {
        async write() { await writing; order.push('write'); },
        async drain() { await writing; order.push('drain'); },
      });
      await call(port, 'open');
      const finished = once(port, 'finish');
      port.end('last packet');
      if (afterFinish) { releaseWrite(); await finished; }
      let settled = false;
      const drained = call(port, 'drain').finally(() => { settled = true; });
  ...
  test(`${name}: drain after end reports a driver failure`, async t => {
    ...
    const port = stream(t, {async drain() { throw failure; }});
    await call(port, 'open');
    const finished = once(port, 'finish');
    port.end('last packet');
    await finished;
