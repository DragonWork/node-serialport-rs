...
const {values} = parseArgs({options: {
  ...
  ports: {type: 'string', default: '1'}, bytes: {type: 'string', default: '65536'},
  mib: {type: 'string', default: '64'}, 'warmup-mib': {type: 'string', default: '4'},
}});
...
class Echo {
  constructor(index) {
    ...
    this.child.on('exit', () => { if (!this.closing) this.fail(new Error('Echo helper exited')); });
    this.stderr = '';
  ...
  async open() {
    ...
    this.port = new SerialPortStream({path: first.value, baudRate: 115200, autoOpen: false, binding});
    this.port.on('error', error => this.fail(error));
    this.port.on('close', () => { if (!this.closing) this.fail(new Error('Serial port closed unexpectedly')); });
    this.port.on('data', data => {
      try {
        ...
        for (let offset = 0; offset < data.length;) {
          ...
          assert(data.subarray(offset, offset + length).equals(this.expected.subarray(position, position + length)), 'Corrupt or reordered echo');
  ...
  async transfer(length) {
    if (!length) return;
    this.target += length;
    const received = new Promise((resolve, reject) => { this.pending = {resolve, reject}; });
    const sending = (async () => {
  ...
  async close() {
    this.closing = true;
    try { if (this.port?.isOpen) await call(this.port, 'close'); }
    finally { this.child.kill(); await this.exited; }
  }
}
...
async function main() {
  ...
  try {
    ...
    console.log(JSON.stringify({label: values.label, node: process.version, platform: process.platform, arch: process.arch,
      ports, bytes, measuredMiBPerPort: measured / 1048576, warmupMiBPerPort: warmup / 1048576,
      elapsedMs: +elapsedMs.toFixed(2), echoMiBps: +(measured * ports / 1048576 / elapsedMs * 1000).toFixed(2),
      cpuMsPerMiB: +((cpu.user + cpu.system) / 1000 / (measured * ports / 1048576)).toFixed(2),
      memoryMiB: {initial: memoryMiB(initial), before: memoryMiB(before), peak: memoryMiB(peak),
        afterTransfer: memoryMiB(afterTransfer), afterClose: memoryMiB(afterClose)},
      activeWorkers, workersAfterClose: workers(), forcedGc: typeof global.gc === 'function',
      nativeSha256: Object.keys(require.cache).filter(file => file.endsWith('.node')).map(hash),
