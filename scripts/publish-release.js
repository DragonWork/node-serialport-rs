...
async function publishRelease(meta, api) {
  ...
  assert(!published || published.dist?.integrity === meta.integrity,
...
function liveApi(meta, archive, repository, token) {
  ...
  const github = (path, options = {}, missing = false) => jsonResponse(`https://api.github.com/repos/${repository}/${path}`,
    {...options, headers: {Accept: 'application/vnd.github+json', Authorization: `Bearer ${token}`,
      'X-GitHub-Api-Version': '2022-11-28', 'Content-Type': 'application/json'}}, missing);
  ...
  return {
    ...
    npmPackage() { return jsonResponse(`https://registry.npmjs.org/${encodeURIComponent(meta.name)}/${encodeURIComponent(meta.version)}`, {}, true); },
    async release() {
    ...
    publishNpm() {
      execFileSync('npm', ['publish', archive, '--ignore-scripts', '--registry=https://registry.npmjs.org',
        '--access=public', '--provenance', '--tag', meta.distTag], {stdio: 'inherit'});
    },
    publishDraft(release) {
      return github(`releases/${release.id}`, {method: 'PATCH', body: JSON.stringify({draft: false,
        make_latest: meta.distTag === 'next' ? 'false' : 'legacy'})});
    },
  };
}
...
if (require.main === module) main(process.argv[2] || join('artifacts', 'release.json'))
