...
function fixture(t, messages = []) {
  ...
  writeFileSync(
    ...
    JSON.stringify(
      { name: 'serialport-rs', version, lockfileVersion: 3, packages: { '': { name: 'serialport-rs', version } } },
      null,
      2,
    ) + '\n',
  );
  ...
  for (const [message, version] of [
    ...
    await t.test(version, t => {
      ...
      for (const name of ['Cargo.toml', 'Cargo.lock'])
        assert(readFileSync(join(root, name), 'utf8').includes(`version = "${version}"`));
...
test('git-cliff renders the requested author, PR and compare-link format', { skip: !cliff }, t => {
  ...
  const context = JSON.parse(
    execFileSync(
      cliff,
      ['--config', join(root, 'cliff.toml'), '--offline', '--unreleased', '--tag', 'v1.12.2', '--context'],
  ...
  const notes = execFileSync(
    cliff,
    ['--config', join(root, 'cliff.toml'), '--offline', '--from-context', path, '--strip', 'all'],
    { cwd: root, encoding: 'utf8' },
  ).trim();
  assert.equal(
    notes,
...
test('preparation records only release files and rejects concurrent branch updates', { skip: !cliff }, async t => {
  ...
  t.mock.method(global, 'fetch', async (url, options) => {
    ...
    return {
      ok: true,
      json: async () => ({
        data: { createCommitOnBranch: { commit: { oid: 'c'.repeat(40), signature: { isValid: true } } } },
      }),
    };
  });
  ...
  t.mock.method(global, 'fetch', async () => ({
    ok: true,
    json: async () => ({
      data: { createCommitOnBranch: { commit: { oid: 'c'.repeat(40), signature: { isValid: false } } } },
    }),
  }));
  await assert.rejects(
    recordPrepared(root, meta, 'DragonWork/node-serialport-rs', 'main', 'test-token'),
    /valid GitHub signature/,
  );
  t.mock.method(global, 'fetch', async () => ({
    ok: true,
    json: async () => ({ errors: [{ message: 'expectedHeadOid does not match' }] }),
  }));
  await assert.rejects(
    recordPrepared(root, meta, 'DragonWork/node-serialport-rs', 'main', 'test-token'),
