...
(async () => {
  ...
  let cases = 0;
  for (const length of [1, 3, 64, 4096]) {
    await compare('ByteLengthParser', { length }, binary);
    cases++;
  }
  for (const delimiter of [Buffer.from([7]), Buffer.from([7, 20, 33]), Buffer.from([0xff, 0xff])]) {
    for (const includeDelimiter of [false, true]) {
      await compare('DelimiterParser', { delimiter, includeDelimiter }, binary);
      cases++;
    }
  }
  ...
  for (const options of [
    ...
    await compare('SlipDecoder', options, encoded);
    cases++;
  }
  for (const maxBufferSize of [3, 64, 65536]) {
    await compare('InterByteTimeoutParser', { interval: 60000, maxBufferSize }, binary);
    cases++;
  }
  const cctalk = Buffer.from([1, 0, 2, 3, 4, 1, 2, 3, 4, 5, 6, 7, 1]);
  await compare('CCTalkParser', 0, cctalk);
  cases++;
  await compare('PacketLengthParser', {}, Buffer.from([0, 0xaa, 3, 4, 5, 6, 0xaa, 2, 7, 8, 0xaa, 3, 9]));
  cases++;
  const options = {
    delimiter: [0xaabc, 0xdef0],
    delimiterBytes: 2,
    lengthOffset: 2,
    lengthBytes: 2,
    packetOverhead: 4,
  };
  await compare('PacketLengthParser', options, Buffer.from([0xaa, 0xbc, 3, 0, 1, 2, 3, 0xde, 0xf0, 1, 0, 4]));
  cases++;
  ...
  for (const options of [{}, { timeCodeFieldLength: 1, ancillaryDataFieldLength: 2 }]) {
    await compare('SpacePacketParser', options, Buffer.concat([space, space, space.subarray(0, 4)]));
    cases++;
  }
