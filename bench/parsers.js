...
async function run([name, Parser, options, input, chunkSize]) {
  ...
  for (let trial = 0; trial < 6; trial++) {
    ...
    outputBytes = 0;
    parser.on('data', chunk => { outputBytes += chunk.length; });
    const end = once(parser, 'end');
    const cpuStart = process.cpuUsage();
    const start = performance.now();
    for (let offset = 0; offset < input.length; offset += chunkSize) parser.write(input.subarray(offset, offset + chunkSize));
    parser.end();
    await end;
    const ms = performance.now() - start;
    const cpu = process.cpuUsage(cpuStart);
    if (trial) { times.push(ms); cpus.push((cpu.user + cpu.system) / 1000); }
  }
  times.sort((a, b) => a - b);
  cpus.sort((a, b) => a - b);
  return {name, medianMs: +times[2].toFixed(2), medianCpuMs: +cpus[2].toFixed(2),
    MiBps: +(input.length / 1048576 / (times[2] / 1000)).toFixed(1), outputBytes};
}
