...
export interface NativeAddon {
  NativePort: new (options: Required<BindingOpenOptions>,
    callback: (error: Error | null, event: Buffer | NativeEvent) => void,
    allocator: (size: number) => Buffer) => NativeHandle;
  listPorts(): Promise<NativePortInfo[]>;
}

interface TargetSelection {platform: string; arch: string; libc?: string; arm?: number; endian?: string;}
...
function loadNative(): NativeAddon {
  ...
  const libc = platform === 'linux' ? ((process.report!.getReport() as {header: {glibcVersionRuntime?: string}}).header.glibcVersionRuntime ? 'gnu' : 'musl') : undefined;
