...
function checkFiles(files) {
  ...
  for (const name of ['index.js', 'index.d.ts', 'lib/targets.json', 'CHANGELOG.md', 'LICENSE', 'NOTICE', 'THIRD_PARTY_LICENSES.md']) {
...
function packageRelease(output = 'artifacts') {
  ...
  const metadata = {name: info.name, version: info.version, tag: `v${info.version}`,
    distTag: info.version.includes('-') ? 'next' : 'latest', commit, filename, notes: prepared.notes,
