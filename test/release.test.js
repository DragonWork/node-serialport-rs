...
test('npm failure leaves the release unpublished and a retry reuses its draft', async () => {
  ...
  const first = provider({async publishNpm() { throw failure; }});
...
test('tag and npm version collisions fail before any release mutation', async () => {
  for (const overrides of [
    {async tagCommit() { return 'c'.repeat(40); }},
    {async npmPackage() { return {dist: {integrity: 'sha512-different'}}; }},
    {async release() { return {draft: true, target_commitish: 'c'.repeat(40)}; }},
  ]) {
...
test('completed immutable releases are verified without rewriting assets or tags', async () => {
  const {api, calls} = provider({
    async tagCommit() { return meta.commit; },
    async npmPackage() { return {dist: {integrity: meta.integrity}}; },
    async release() { return {draft: false, assets: [{name: meta.filename, digest: `sha256:${meta.sha256}`}]}; },
  });
  await publishRelease(meta, api);
  assert.deepEqual(calls, []);
});
